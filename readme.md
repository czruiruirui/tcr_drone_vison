...
## 3 详细信息
### 3.1 项目环境
操作系统：Ubuntu 22.04（JetPack，aarch64）\
运算平台：Jetson Orin NX\
...
推理框架：TensorRT（CUDA / cuDNN 随 JetPack 提供）\
辅助工具：NoMachine（远程桌面）、PlotJuggler（绘制曲线图）

### 3.2 编译与部署
1. 安装依赖项：
   - JetPack：CUDA、cuDNN、TensorRT 均随 JetPack 提供，CMake 检测到 TensorRT 后会自动定义 `USE_TENSORRT` 并启用 GPU 推理
   - 相机 SDK：海康 / 迈德威视 SDK 已随仓库附带（`io/hikrobot`、`io/mindvision` 下的 `include` 与 `lib/arm64`），无需单独安装
   - 其余：
    ```bash
    sudo apt install -y \
        git \
...
2. 准备神经网络模型（TensorRT engine）：
   `.engine` 文件与 GPU 架构和 TensorRT 版本绑定，**必须在部署的 Orin NX 本机上生成**，换机或重刷 JetPack 后需要重新生成：
    ```bash
    /usr/src/tensorrt/bin/trtexec --onnx=assets/your_model.onnx --saveEngine=assets/your_model.engine --fp16
    ```
    生成后在 `configs/*.yaml` 中把 `yolov5_model_path`、`classify_model` 指向对应的 `.engine` 文件（如 `assets/0526.engine`、`assets/tiny_resnet.engine`）。
...
4. 运行（注意：yaml 中的模型路径是相对路径，需在仓库根目录下启动）：
    ```bash
    ./build/auto_aim_debug_mpc configs/standard3.yaml   # 自瞄（轨迹规划器）
    ./build/mt_standard configs/standard3.yaml          # 多线程自瞄 + 打符
    ```
    - 程序每 100 帧在日志中打印一次平均 FPS 和 detect 平均耗时，可用于性能确认
    - yaml 中 `show_debug: true` 可打开检测可视化窗口，默认为 `false`；Jetson 上 `imshow` 每帧开销可达 10ms 级，实测/比赛时请保持关闭
...
5. 注册自启：
    ...
    3. 在该文件中写入:
        ...
        Name=sp_vision
        ```
        注: [Exec](https://specifications.freedesktop.org/desktop-entry-spec/desktop-entry-spec-latest.html)必须为绝对路径，并同步修改 `autostart.sh` 中 `cd` 的项目路径为实际部署路径。
...
7. 串口设置
    ...
    3. 创建 udev 规则文件
        ...
        ```SUBSYSTEM=="tty", ATTRS{idVendor}=="0483", ATTRS{idProduct}=="5740", ATTRS{serial}=="335235703032", SYMLINK+="gimbal"
    4. 重新加载 udev 规则
        ```
